...
import random	#PNT
def other_random(a,b):
	return random.uniform(a,b)

class LunarLanderMod(gym.Env, EzPickle):	#PNT
	r"""
	## Description
	...
	* The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
	...
	def _create_wind_particle(self, mass, x, y, ttl):	#PNT
		p = self.world.CreateDynamicBody(
			position=(x, y),
			angle=0.0,
			fixtures=fixtureDef(
		...
		p.ttl = ttl
		colorA = other_random(0.1, 0.5)
		colorB = other_random(0.1, 0.5)
		p.color2 = (
			int(colorA * 64),
			int(colorA * 64),
			int((colorA+colorB) * 64),
		)
		self.wind_particles.append(p)
		self._clean_particles(False)
		return p
	...
	def _clean_particles(self, all_particle):
		...
		while self.wind_particles and (all_particle or self.wind_particles[0].ttl < 0):	#PNT
			self.world.DestroyBody(self.wind_particles.pop(0))
	...
	def step(self, action):
		...
		if self.enable_wind and not (
			...
			# wind particles	#PNT
			wind_particle_speed = 0.2
			wind_ttl = 2
			for i in np.arange(4):
				wind_particle_pos = (self.lander.position[0] + 0.1*wind_mag*wind_ttl*wind_particle_speed,
									self.lander.position[1] + other_random(-0.5, 0.5))
				p = self._create_wind_particle(
					5,
					wind_particle_pos[0], wind_particle_pos[1],
					wind_ttl,
				)  # particles are just a decoration
				p.ApplyLinearImpulse(
					(wind_mag*(wind_particle_speed*other_random(0.9, 1.1)), 0),
					wind_particle_pos,
					True,
				)
	...
	def render(self):
		...
		for obj in self.wind_particles:	#PNT
			obj.ttl -= 0.15
			# obj.color2 = (
				# int(max(0.1, 0.3 * obj.ttl) * 64),
				# int(max(0.1, 0.3 * obj.ttl) * 64),
				# int(max(0.1, 0.3 * obj.ttl) * 128),
			# )
		...
		for obj in self.wind_particles + self.particles + self.drawlist:	#PNT

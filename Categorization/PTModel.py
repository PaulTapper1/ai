...
class NeuralNetwork(nn.Module):
    def __init__(self, settings):
        ...
        #     )
        # # got up to 89% after 360 epochs
        
        # # see https://docs.determined.ai/0.27.0/tutorials/pytorch-mnist-tutorial.html for different architecture
        # self.linear_relu_stack = nn.Sequential(
            # nn.Conv2d(1, 64, kernel_size=3, padding=1),  # Input: 1 channel (grayscale), Output: 64 channels
            # nn.ReLU(),
            ...
            # nn.Flatten(),
            # nn.Linear(64 * 28//4 * 28//4, 256),
            # nn.ReLU(),
            # nn.Linear(256, 10),
            # #nn.LogSoftmax(),
            # )
        # # got up to 91% after 300 epochs

        conv2d_0 = settings[0]
        conv2d_1 = settings[1]
        linear_0 = settings[2]
        self.linear_relu_stack = nn.Sequential(
            nn.Conv2d(1, conv2d_0, kernel_size=3, padding=1),  # Input: 1 channel (grayscale), Output: 32 channels
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),  # Downsample by 2
            nn.Conv2d(conv2d_0, conv2d_1, kernel_size=3, padding=1),  # Input: 32 channels, Output: 16 channels
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),   # Downsample by 2 again
            nn.Flatten(),

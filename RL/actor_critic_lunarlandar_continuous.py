...
class PolicyEstimator:
    ...
    def predict(self, state, sess):
        ...
        # Return actions for both main and side engines
        # using self.mu and self.sigma from the model.
        mu, sigma = sess.run([self.mu, self.sigma], feed_dict=feed_dict)  
        # Create a normal distribution using mu and sigma
        dist = tf.compat.v1.distributions.Normal(mu, sigma)  
        # Sample two actions from the distribution for main and side engines
        actions = dist.sample(2)
        # Clip the actions within the environment's action space limits
        actions = tf.clip_by_value(actions, self.env.action_space.low, self.env.action_space.high)
        # Execute the TensorFlow operations and convert to a NumPy array
        actions = actions.eval(session=sess)  
        # Return the actions as a list with two elements for the environment to use
    ...
    with tf.compat.v1.Session() as sess: # Use tf.compat.v1.Session() for compatibility with TF 1.x code
        ...
        for i_episode in range(episodes):
            ...
            for t in itertools.count():
                ...
                step_result = env.step(action) # extract the action from the list
